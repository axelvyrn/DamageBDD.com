...
Welcome to DamageBDD, the powerful and user-friendly behavior testing platform designed to help you automate your application
testing process. In this quick start guide, we'll walk you through setting up an account, generating a simple test, and running
it using cURL. Let's dive in!


** Create an Account

To get started with DamageBDD, you need to create an account. You can do this by sending a POST request to our account creation
endpoint using the `[[https://curl.se/][curl]]` command:
...
You can also use this [[./accounts.org][form]] to create an account from a browser.
    
** Get a Bearer Token

Next, you need to get a bearer token by authenticating with our system:
...
** Running tests against Authorized Domains
...
curl -X GET \
    -H "Authorization: Bearer "$AUTH_TOKEN \
    https://run.damagebdd.com/accounts/domains -H "Content-Type: application/json"
</code></pre>
#+END_EXPORT
...
curl -X PUT \
    -H "Authorization: Bearer "$AUTH_TOKEN \
    https://run.damagebdd.com/accounts/domains -H "Content-Type: application/json" -d '{"domain": "example.com"}'
</code></pre>
#+END_EXPORT
...
[[./run.org][Here]] is the link to the experimental web interface, using curl is the recommended method.
